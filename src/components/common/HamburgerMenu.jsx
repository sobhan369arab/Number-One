...
const HamburgerMenu = ({ children, style, width }) => {
    ...
    return (
        <div>
            ...
            <div onClick={() => { setVisible(!visible) }} style={{ opacity: visible ? "1" : "0", visibility: visible ? "visible" : "hidden" }} className="w-full duration-250 h-svh fixed top-0 right-0 bg-black/55 z-30">
                <div onClick={() => { setVisible(visible) }} style={i18n.language != "en" ? { right: visible ? "0" : - width + "px" } : { left: visible ? "0" : - width + "px" }} className={`w-[${width + "px"}] h-svh duration-200 z-50 fixed top-0 select-none  ${style}`}>
