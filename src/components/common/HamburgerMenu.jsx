...
const HamburgerMenu = ({ children, style, width }) => {
    ...
    return (
        <div>
            ...
            <div style={i18n.language != "en" ? { right: visible ? "0" : - width + "px" } : { left: visible ? "0" : - width + "px" }} className={`w-[${width + "px"}] h-svh duration-200 z-50 fixed top-0 ${style}`}>
                <div onClick={() => { setVisible(!visible) }} className={`${i18n.language != "en" ? "right-6" : "left-6"} absolute top-6 cursor-pointer hover:scale-105 duration-200 w-8 h-8 bg-yellowCustom rounded-full flex justify-center items-center`}><CloseIcon /></div>
                {children}
            </div>
        </div>
    )
